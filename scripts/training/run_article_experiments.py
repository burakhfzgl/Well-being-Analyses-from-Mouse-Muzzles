"""Run article-grade mouse impairment ablation experiments.

Examples:
    python scripts/training/run_article_experiments.py --part part1
    python scripts/training/run_article_experiments.py --part part1 --dry-run
    python scripts/training/run_article_experiments.py --part all --only convnext_tiny_crop_light
"""
...
from balanced_device import (  # noqa: E402
    BalancedDeviceConfig,
    build_balanced_split,
    config_from_dict,
    run_balanced_device_training,
)
from paths import FIGURES_DIR, IMAGES_MGS_DIR, MODELS_DIR, MOUSE_DATASET_DIR, REPORTS_DIR  # noqa: E402
...
def remap_to_original(split_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise RuntimeError(f"{len(missing)} split images do not exist in images_mgs; first missing: {missing[:5]}")
    return mapped
...
def iter_experiments(part: str, only: set[str] | None = None) -> Iterable[dict]:
    base_config = config_from_dict(BASE_SETTINGS | {"split_csv": ARTICLE_SPLITS_DIR / "heldout_subset_class_crop.csv"})
...
def write_manifest(experiments: list[dict]) -> None:
    ...
    manifest_path = ARTICLE_REPORTS_DIR / "experiment_manifest.json"
    with manifest_path.open("w", encoding="utf-8") as file:
        json.dump([{key: _json_ready(value) for key, value in item.items()} for item in experiments], file, indent=2)
    pd.DataFrame([{key: _json_ready(value) for key, value in item.items()} for item in experiments]).to_csv(
        ARTICLE_REPORTS_DIR / "experiment_manifest.csv",
        index=False,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Prepare splits and print commands without training.")
    parser.add_argument("--only", nargs="*", help="Run only these run names.")
    parser.add_argument("--limit-runs", type=int, default=None, help="Run only the first N selected experiments.")
...
def main() -> None:
    ...
    experiments = list(iter_experiments(args.part, only=only))
    if args.limit_runs is not None:
        experiments = experiments[: args.limit_runs]
    ...
    for settings in experiments:
        print(f"- {settings['run_name']}: {settings['model_name']} | {settings['source_mode']} | {settings['split_csv']}")
